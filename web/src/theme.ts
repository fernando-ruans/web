import './index.css';

// Tema principal customizado para delivery
const theme = {
  bg: 'bg-gradient-to-br from-yellow-50 via-yellow-100 to-orange-100 min-h-screen',
  card: 'bg-white/90 rounded-xl shadow-lg p-8',
  primary: 'bg-orange-500 hover:bg-orange-600 text-white',
  secondary: 'bg-yellow-400 hover:bg-yellow-500 text-gray-900',
  accent: 'text-orange-500',
  border: 'border border-orange-200',
  input: 'bg-gray-100 border border-orange-200 rounded px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-400',
  title: 'text-3xl font-extrabold text-orange-500 mb-2',
  subtitle: 'text-lg text-gray-700 mb-4',
};

export default theme;